...
<div x-data="formModal({
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold text-strong">Mascotas</h1>
            <p class="text-sm text-muted">Pacientes de la veterinaria.</p>
        </div>
                ...
                <tbody>
                    <template x-for="row in rows" :key="row.id">
                        <tr>
                            ...
                            <td class="text-muted" x-text="row.breed || '—'"></td>
                            <td x-text="row.owner_name || '—'"></td>
                            <td class="whitespace-nowrap text-muted" x-text="$store.app.fmtDate(row.birth_date)"></td>
                            <td class="whitespace-nowrap text-muted" x-text="row.weight_kg ? row.weight_kg + ' kg' : '—'"></td>
                            <td>
                    ...
                    <tr x-show="!loading && rows.length === 0">
                        <td colspan="7" class="py-10 text-center text-sm text-faint">Sin resultados.</td>
                    </tr>
                    <tr x-show="loading">
                        <td colspan="7" class="py-10 text-center text-sm text-faint">Cargando…</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="modal-panel p-6" x-show="open" x-transition.scale.origin.center @keydown.escape.window="close()">
            <h3 class="text-base font-semibold text-strong" x-text="mode === 'create' ? 'Nueva mascota' : 'Editar mascota'"></h3>
