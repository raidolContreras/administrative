...
<div x-data="formModal({
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold text-strong">Usuarios</h1>
            <p class="text-sm text-muted">Personas con acceso al panel.</p>
        </div>
                ...
                <tbody>
                    <template x-for="row in rows" :key="row.id">
                        <tr>
                            ...
                            <td class="whitespace-nowrap text-muted" x-text="$store.app.fmtDateTime(row.last_login_at)"></td>
                            <td>
                                <div class="flex justify-end gap-1">
                                    ...
                                    <button class="btn-icon hover:!text-danger" title="Eliminar" @click="destroy(row, 'al usuario ' + row.name)">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <tr x-show="!loading && rows.length === 0">
                        <td colspan="6" class="py-10 text-center text-sm text-faint">Sin resultados.</td>
                    </tr>
                    <tr x-show="loading">
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="modal-panel p-6" x-show="open" x-transition.scale.origin.center @keydown.escape.window="close()">
            <h3 class="text-base font-semibold text-strong"
