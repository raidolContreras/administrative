...
<div x-data="{
    ...
    <div class="mb-6">
        <h1 class="text-xl font-bold text-strong">Auditoría</h1>
        <p class="text-sm text-muted">Registro de todas las altas, cambios y bajas del sistema.</p>
    </div>
                ...
                <tbody>
                    <template x-for="row in rows" :key="row.id">
                        <tr>
                            ...
                            <td class="text-muted" x-text="row.entity_type + ' #' + row.entity_id"></td>
                            <td class="text-faint" x-text="row.ip || '—'"></td>
                            <td class="text-right">
                    ...
                    <tr x-show="!loading && rows.length === 0">
                        <td colspan="6" class="py-10 text-center text-sm text-faint">Sin registros.</td>
                    </tr>
                    <tr x-show="loading">
    ...
    <div x-show="detail" x-cloak class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="modal-panel max-w-2xl p-6" x-show="detail" x-transition.scale.origin.center @keydown.escape.window="detail = null">
            <h3 class="text-base font-semibold text-strong">Detalle del cambio</h3>
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <p class="label">Valores anteriores</p>
                    <pre class="max-h-64 overflow-auto rounded-lg bg-subtle p-3 text-xs text-body"
